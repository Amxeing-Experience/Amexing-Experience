// Smoke E2E — "Agregar a cotización" desde el catálogo de Tours.
//
// Verifica el flujo real (UI → API) con una sesión autenticada:
//   1. El catálogo de tours renderiza cards con el botón "Agregar" (.add-tour-to-quote-btn).
//   2. Al agregar + "Crear nueva cotización" se crea una cotización y redirige a ella.
//   3. La cotización creada contiene el subconcepto de tour con los datos correctos
//      (type='tour', tourId, total) que arma el módulo genérico add-to-quote.
//
// El add-to-quote de tours SOLO está disponible para department_manager y client
...
test.describe('Agregar a cotización — Tours', () => {
  test.skip(!EMAIL || !PASSWORD, 'Faltan E2E_EMAIL / E2E_PASSWORD');
  test.skip(ROLE === 'admin', 'El add-to-quote de tours no aplica para admin (solo department_manager/client)');

  test('crea cotización desde una card de tour y guarda el subconcepto correcto', async ({ page }) => {
    // --- Login ---
    ...
    // --- Catálogo de tours: esperar cards con botón "Agregar" ---
    ...
    // Capturar los datos del tour que el front usará para el subconcepto.
    const tour = await addBtn.evaluate((el) => ({
      id: el.dataset.tourId,
      name: el.dataset.tourName,
      price: parseFloat(el.dataset.tourPrice) || 0,
    }));
    expect(tour.id, 'la card debe tener data-tour-id').toBeTruthy();
    ...
    // --- Verificar por API que el subconcepto de tour quedó bien ---
    ...
    const tourSub = subs.find((s) => s.type === 'tour');

    expect(tourSub, 'la cotización debe contener un subconcepto de tour').toBeTruthy();
    expect(tourSub.tourId, 'el tourId del subconcepto debe coincidir con la card').toBe(tour.id);
    expect(Number(tourSub.total), 'el total del subconcepto debe coincidir con el precio de la card')
      .toBeCloseTo(tour.price, 2);
    expect(tourSub.isPerPerson, 'el tour se marca por persona').toBe(true);
