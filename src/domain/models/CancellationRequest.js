...
class CancellationRequest extends BaseModel {
  ...
  // -----------------------------------------------------------------
  // Cancellation policy outputs (CancellationPolicyService.calculate):
  // credito (saldo a favor), penalizacion (retenido), reembolso (a tarjeta).
  // Invariant: creditoAmount + penalizacionAmount + reembolsoAmount === montoPagado.
  // refundAmount/cancellationFee above mirror reembolso/penalizacion for back-compat.
  // -----------------------------------------------------------------

  /**
   * Get the credit amount (saldo a favor for future use, not refunded to card).
   * @returns {number} Credit amount.
   * @example
   * const credito = request.getCreditoAmount();
   */
  getCreditoAmount() {
    return this.get('creditoAmount') || 0;
  }

  /**
   * Set the credit amount.
   * @param {number} creditoAmount - Credit kept as future balance.
   * @example
   * request.setCreditoAmount(500);
   */
  setCreditoAmount(creditoAmount) {
    this.set('creditoAmount', creditoAmount);
  }

  /**
   * Get the penalty amount retained by the company.
   * @returns {number} Penalty amount.
   * @example
   * const penalizacion = request.getPenalizacionAmount();
   */
  getPenalizacionAmount() {
    return this.get('penalizacionAmount') || 0;
  }

  /**
   * Set the penalty amount.
   * @param {number} penalizacionAmount - Amount retained by the company.
   * @example
   * request.setPenalizacionAmount(500);
   */
  setPenalizacionAmount(penalizacionAmount) {
    this.set('penalizacionAmount', penalizacionAmount);
  }

  /**
   * Get the refund amount returned to the original payment method.
   * @returns {number} Refund amount.
   * @example
   * const reembolso = request.getReembolsoAmount();
   */
  getReembolsoAmount() {
    return this.get('reembolsoAmount') || 0;
  }

  /**
   * Set the refund amount returned to the original payment method.
   * @param {number} reembolsoAmount - Amount refunded to card.
   * @example
   * request.setReembolsoAmount(1000);
   */
  setReembolsoAmount(reembolsoAmount) {
    this.set('reembolsoAmount', reembolsoAmount);
  }

  /**
   * Get the policy tier that produced the split.
   * @returns {string} Tier (empresa|no_show|mayor_igual_24h|entre_12_24h|menor_12h).
   * @example
   * const tier = request.getPolicyTier();
   */
  getPolicyTier() {
    return this.get('policyTier');
  }

  /**
   * Set the policy tier.
   * @param {string} policyTier - Computed policy tier.
   * @example
   * request.setPolicyTier('entre_12_24h');
   */
  setPolicyTier(policyTier) {
    this.set('policyTier', policyTier);
  }

  /**
   * Get whether the client no-showed.
   * @returns {boolean} True if no-show.
   * @example
   * const noShow = request.getEsNoShow();
   */
  getEsNoShow() {
    return this.get('esNoShow') === true;
  }

  /**
   * Set whether the client no-showed.
   * @param {boolean} esNoShow - True if the client did not show up.
   * @example
   * request.setEsNoShow(true);
   */
  setEsNoShow(esNoShow) {
    this.set('esNoShow', esNoShow === true);
  }

  /**
   * Get who cancelled (cliente or empresa).
   * @returns {string} 'cliente' | 'empresa'.
   * @example
   * const tipo = request.getTipoCancelacion();
   */
  getTipoCancelacion() {
    return this.get('tipoCancelacion') || 'cliente';
  }

  /**
   * Set who cancelled (cliente or empresa). Drives the policy: empresa = 100% refund.
   * @param {string} tipoCancelacion - 'cliente' | 'empresa'.
   * @example
   * request.setTipoCancelacion('empresa');
   */
  setTipoCancelacion(tipoCancelacion) {
    this.set('tipoCancelacion', tipoCancelacion);
  }
  ...
  validate(attrs) {
    ...
    // Valid cancellation initiators (who cancels — drives the refund policy)
    const validInitiators = ['cliente', 'empresa'];
    const tipoCancelacion = attrs.tipoCancelacion || this.get('tipoCancelacion');
    if (tipoCancelacion && !validInitiators.includes(tipoCancelacion)) {
      return new Parse.Error(Parse.Error.VALIDATION_ERROR, `Invalid cancellation initiator: ${tipoCancelacion}`);
    }
